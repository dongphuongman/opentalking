...
| QuickTalk weights | `$DIGITAL_HUMAN_HOME/models/quicktalk/checkpoints` |
| OmniRT QuickTalk root | Point to `.../checkpoints` |
| Benchmark compatibility path | Use `$DIGITAL_HUMAN_HOME/models/quicktalk` directly |
...
Stop the OpenTalking API, WebUI, and OmniRT processes started by quickstart scripts
or `scripts/start_unified.sh`:
...
If `omnirt serve-avatar-ws ...` was started manually in the foreground, press
`Ctrl+C` in that WSL2 terminal. To fully restart WSL2, run this from Windows
PowerShell:
